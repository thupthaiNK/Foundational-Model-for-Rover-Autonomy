"""
Purpose: Live Gazebo verification of "mission-level failsafe reaction"
         (item 12, grill-scoped 2026-07-20): a reactive_explorer terminal
         FAILSAFE (it has given up on a specific hazard but the rover is
         still mobile, unlike stuck_detection's FAILSAFE) autonomously
         aborts the current frontier exploration and drives back to the
         rover's own recorded start pose instead of holding zero Twist
         forever. Deliberately does NOT run reactive_explorer_node at all
         -- the planner cannot tell an injected /reactive_explorer/failsafe
         signal from a genuine one (it is a bare Bool topic), and
         reactive_explorer's own hazard-triggering logic is already
         verified elsewhere (28 unit tests + live, §4.8.17-18); this
         isolates the planner's abort-to-home response as the sole thing
         under test, deterministically, rather than depending on staging a
         Gazebo physical condition that makes reactive_explorer itself give
         up (see §4.8.25's "ROS2-live stimulus instead of staged Gazebo
         physics" precedent for the same reasoning). Otherwise identical to
         frontier_exploration_test.launch.py: spawn at the box centre
         (-6, 6) in soil_zone, costmap with init_unknown:=true, planner in
         frontier_mode with abort_to_home:=true (return_home left at its
         default False -- the two are independent per the grill decision).
Inputs:  None (launches everything automatically; the failsafe injection is
         performed by experiments/abort_to_home_test.py, not this file).
