"""
Purpose: Live Gazebo verification of "L6-lite" (scoped via grill-thesis
         2026-07-18) -- the smallest possible sliver of L6 (Mission
         Autonomy), built on top of L5-lite once it first reached a goal
         end-to-end (§4.8.26). Identical to l5_lite_test.launch.py in every
         respect except one: l5_lite_planner_node.py's new waypoint_xs/
         waypoint_ys parameters are set so the rover autonomously advances
         from D1's real GOAL_POSE (-7.5, -9.0, the default first waypoint)
         to D1's real START_POSE (7.5, 1.0) once GOAL_POSE is reached, with
         no operator command in between -- a recon-and-return mission using
         the same two points D1 and L5-lite already established.
...
How to run:
    ...
    ros2 launch simulation/launch/l6_lite_roundtrip_test.launch.py
...
def generate_launch_description():
    ...
    # -- L6-lite: same L5-lite planner, but with a second waypoint (D1's
    # START_POSE) so the rover autonomously returns once GOAL_POSE (the
    # default first waypoint) is reached, with no operator command.
