...
CREEP_VERIFY_SPEED = 0.02  # m/s -- slower than every nonzero POLICY speed
                            # (bedrock=0.03 is the slowest), so approach-to-
                            # verify never drives faster than the most
                            # cautious confident-classification speed already
                            # deployed.
CREEP_VERIFY_MAX_DISTANCE_M = 1.5  # matches X3's finding (§4.8.10) that
                                     # rocks become reliably detectable within
                                     # ~2-3m -- capped short of the far edge
                                     # of that range so the rover reaches a
                                     # closer, more resolving viewpoint but
                                     # never approaches all the way to an
                                     # object that turns out to be a genuine
                                     # hazard.


def should_creep_to_verify(label: str, creep_distance_m: float,
                            max_creep_distance_m: float = CREEP_VERIFY_MAX_DISTANCE_M) -> bool:
    """Approach-to-verify creep (item 23, grill-scoped 2026-07-20): turns
    X3's finding that rock detection is position-specific and only reliable
    within ~2-3m (§4.8.10) into a live behaviour. On an "uncertain" label,
    instead of A4's stop-in-place vote (§4.8.31 H19), creep forward at
    CREEP_VERIFY_SPEED so the closer viewpoint has a chance to resolve the
    classification -- but only up to max_creep_distance_m. True while still
    uncertain and under the cap; False once the label resolves to anything
    else, OR once the cap is reached (the caller then falls through to the
    existing STOP path -- this predicate can only delay a STOP decision by a
    bounded distance, never remove it, matching every prior uncertain-
    handling result's safety-first design)."""
    return label == "uncertain" and creep_distance_m < max_creep_distance_m


def creep_verify_speed() -> float:
    return CREEP_VERIFY_SPEED


def two_stage_vote_verdict(votes, window):
    """A4 two-stage uncertain policy (grill-scoped 2026-07-19). After the
    rover STOPs on an uncertain classification (immediately -- safety
    unchanged), it keeps classifying while stopped and collects the labels
    into `votes`. This decides, from the most recent `window` of them,
    whether that STOP was a single-frame false alarm:
      - "gathering": fewer than `window` votes so far, no decision yet;
      - "false_stop": a STRICT majority of the last `window` votes are
        traversable (soil/sand/bedrock, i.e. non-zero policy speed) -- the
        first uncertain frame was noise, the STOP may be released early;
      - "confirmed_stop": otherwise -- ties and stop-labels
        (uncertain/big_rock/unknown) keep the rover stopped, safety-first.
    Pure function: no side effects, no dependency on node state, so the
    false-STOP reduction it drives is measurable and unit-testable in
    isolation."""
    if len(votes) < window:
        return "gathering"
    recent = votes[-window:]
    traversable = sum(1 for v in recent if POLICY.get(v, 0.0) > 0.0)
    return "false_stop" if traversable * 2 > window else "confirmed_stop"
...
class TerrainControllerNode(Node):
    ...
    def __init__(self):
        ...
        # A4 two-stage uncertain policy (opt-in, default 0=off -- zero
        # behaviour change from every prior result). When >0, on an
        # uncertain STOP the rover keeps classifying while stopped and, if a
        # strict majority of the next N frames are traversable, treats the
        # STOP as a single-frame false alarm and releases early -- STOP is
        # always entered first (safety unchanged), only its release is
        # accelerated. Counts the false-STOPs it prevents for measurement.
        self.declare_parameter("uncertain_vote_window", 0)
        ...
        self.uncertain_vote_window = (
            self.get_parameter("uncertain_vote_window").get_parameter_value().integer_value
        )
        ...
        # A4 two-stage state
        self._uncertain_votes = []       # labels seen since the current uncertain STOP began
        self._in_uncertain_stop = False  # True while stopped specifically due to uncertain
        self._false_stops_prevented = 0  # measurement: uncertain STOPs the vote released early
    ...
    def _terrain_callback(self, msg: String) -> None:
        ...
        # A4 two-stage uncertain policy (opt-in). Runs only in discrete mode
        # (continuous-score mode sets speed via _score_callback and does not
        # use the uncertain-label STOP path). STOP is always entered above;
        # this can only *release* an uncertain STOP early once a strict
        # majority of the votes gathered while stopped are traversable.
        vote_released = False
        if self.uncertain_vote_window > 0 and not self.use_continuous_score:
            if label == "uncertain":
                if not self._in_uncertain_stop:
                    self._in_uncertain_stop = True
                    self._uncertain_votes = []
                self._uncertain_votes.append(label)
            elif self._in_uncertain_stop:
                # Still voting on a live uncertain STOP: this (possibly
                # traversable) frame is a vote.
                self._uncertain_votes.append(label)
                verdict = two_stage_vote_verdict(
                    self._uncertain_votes, self.uncertain_vote_window)
                if verdict == "false_stop":
                    self._false_stops_prevented += 1
                    self._stop_latch_until_ns = 0  # release the latch early
                    self._in_uncertain_stop = False
                    self._uncertain_votes = []
                    vote_released = True
                    self.get_logger().info(
                        f"A4: uncertain STOP released early by vote "
                        f"(false-STOPs prevented so far: "
                        f"{self._false_stops_prevented})"
                    )
                elif verdict == "confirmed_stop":
                    self._in_uncertain_stop = False
                    self._uncertain_votes = []

        latch_active = (now_ns < self._stop_latch_until_ns) and not vote_released
