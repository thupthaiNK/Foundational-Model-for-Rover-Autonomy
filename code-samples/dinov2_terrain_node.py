...
# Default cache path — override via ROS2 parameter
_THIS_DIR    = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT   = os.path.normpath(os.path.join(_THIS_DIR, "..", "..", "..", "..", ".."))
_DEFAULT_CACHE = os.path.join(
    _REPO_ROOT, "experiments", "results", "feature_cache", "dinov2_reg_small_train_1000shot.npz"
)
_DEFAULT_ONNX_ENCODER = os.path.join(
    _REPO_ROOT, "experiments", "results", "dinov2_reg_small_encoder_int8.onnx"
)
...
class DINOv2TerrainNode(Node):
    ...
    def __init__(self):
        ...
        # ── Load DINOv2 ViT-S encoder (image processor is always needed --
        #    it's tiny/fast and shared by both backends below) ─────────────
        ...
        if self.use_int8_model:
            if not os.path.exists(onnx_encoder_path):
                raise FileNotFoundError(
                    f"INT8 ONNX encoder not found: {onnx_encoder_path}\n"
                    f"Run experiments/int8_quantization_rpi.py first, or set "
                    f"use_int8_model:=false to fall back to the FP32 torch model."
                )
    ...
    def _train_logreg(
        ...
        if not os.path.exists(cache_path):
            raise FileNotFoundError(
                ...
                f"Run experiments/dinov2_terrain_test.py first to generate the cache."
            )
        ...
        # Sample n_shot per class (deterministic — same protocol as offline experiments)
        idx = []
        ...
        self.get_logger().info(
            f"LogReg trained on {len(idx)} samples "
            f"({dict(zip(*np.unique(y, return_counts=True)))}) in {elapsed:.1f}s | "
            f"class_weight={class_weight or 'none'}"
        )
        return clf
    ...
    def _extract_features_onnx(self, pil_img: PILImage.Image) -> np.ndarray:
        ...
        inputs = self.processor(images=pil_img, return_tensors="np")
