"""
Purpose: Few-shot linear probe on frozen DINOv3 ViT-B/16 features using AI4Mars dataset.
         Tests whether scaling DINOv3 from ViT-S (21.6M, 384-d, 90.2%) to ViT-B (86M, 768-d)
         closes the gap to supervised (96.67%) — placed between ViT-S (90.2%) and ViT-L (92.3%).
...
Outputs: Per-class accuracy by shot count, CSV saved to results/
         Feature cache saved to results/feature_cache/dinov3_vitb_*
How to run:
    python3 -u experiments/dinov3_vitb_terrain_test.py | tee /tmp/dinov3_vitb_log.txt
    python3 -u experiments/dinov3_vitb_terrain_test.py --shots 1000  # 1000-shot only
    python3 -u experiments/dinov3_vitb_terrain_test.py --shots 10    # smoke test (~5 min)
Note: Requires HuggingFace token (gated model). Same gating as ViT-S/ViT-L — access approved.
      Expected runtime: ~3–4 hours on laptop CPU (~2s/img for 3108 train + 287 test images)
...
def load_model(device: str):
    print(f"Loading DINOv3 ViT-B/16 (frozen)  [{MODEL_ID}]")
    print(f"  Expected: ~86M params, 768-d features — ~3–4h runtime")
...
def extract_features(model, processor, image_paths, labels, device: str, desc: str = ""):
    """Extract L2-normalised DINOv3 ViT-B CLS-token features."""
    features = []
    n  = len(image_paths)
    t0 = time.perf_counter()
...
def print_results(results: dict):
    ...
    print("\n" + "=" * 110)
    print("DINOv3 ViT-B/16 Few-Shot Linear Probe — AI4Mars (287 test images)")
    print("Scaling: ViT-S (21.6M, 384-d, 90.2%) → ViT-B (86M, 768-d) → ViT-L (303M, 1024-d, 92.3%)")
    print("=" * 110)
    ...
    for metric in ["overall"] + CLASS_NAMES:
        ...
        for s in shots:
            ...
            vl_val = (DINOV3_VITL_BASELINES.get(s, {}).get("overall")
                      if metric == "overall"
            ...
            row   += f"  [{s:>4}shot] ViT-B:{vb_str} ViT-S:{vs_str} Δ:{d_vs} ViT-L:{vl_str}"
        print(row)

    print("=" * 110)
    print("\nScaling curve — Bedrock accuracy (main thesis contribution):")
    for s in shots:
        vb_b = results[s]["per_class"].get("bedrock")
        vs_b = DINOV3_VITS_BASELINES.get(s, {}).get("bedrock")
        vl_b = DINOV3_VITL_BASELINES.get(s, {}).get("bedrock")
        if vb_b is not None:
            d_vs = f"Δ_vs_ViT-S {vb_b - vs_b:+.1f}%" if vs_b is not None else ""
            vl_s = f"ViT-L {vl_b:.1f}%" if vl_b is not None else "ViT-L N/A"
            print(f"  {s:>5}-shot:  ViT-S {vs_b or 0:.1f}%  →  ViT-B {vb_b:.1f}%  {d_vs}  |  {vl_s}")

    if 1000 in results:
        vb_ov = results[1000]["overall"]
        vs_ov = DINOV3_VITS_BASELINES[1000]["overall"]
        vl_ov = DINOV3_VITL_BASELINES[1000]["overall"]
        print(f"\n>>> 1000-shot Overall: ViT-S {vs_ov:.2f}%  →  ViT-B {vb_ov:.2f}%  "
              f"(Δ_vs_ViT-S {vb_ov - vs_ov:+.2f}%)  |  ViT-L {vl_ov:.2f}%  "
              f"|  Gap vs supervised: {vb_ov - SUPERVISED_BASELINE:+.2f}%")


def save_csv(results: dict, avg_ms_extract: float):
    os.makedirs(RESULTS_DIR, exist_ok=True)
    path = os.path.join(RESULTS_DIR, "dinov3_vitb_terrain_few_shot.csv")
    shots = sorted(results.keys())

    fieldnames = (["shots", "overall", "gap_vs_supervised"] + CLASS_NAMES +
                  ["vits_overall", "vits_bedrock", "vitl_overall", "vitl_bedrock",
                   "delta_vs_vits_overall", "delta_vs_vits_bedrock", "avg_ms_extract"])

    rows = []
    for s in shots:
        r  = results[s]
        vs = DINOV3_VITS_BASELINES.get(s, {})
        vl = DINOV3_VITL_BASELINES.get(s, {})
        ...
        row["vits_overall"]          = f"{vs.get('overall', 0):.2f}" if vs.get("overall") else "N/A"
        row["vits_bedrock"]          = f"{vs_br:.2f}" if vs_br is not None else "N/A"
        row["vitl_overall"]          = f"{vl.get('overall', 0):.2f}" if vl.get("overall") else "N/A"
        row["vitl_bedrock"]          = f"{vl.get('bedrock', 0):.2f}" if vl.get("bedrock") else "N/A"
        row["delta_vs_vits_overall"] = (f"{r['overall'] - vs['overall']:+.2f}"
                                        if vs.get("overall") is not None else "N/A")
        row["delta_vs_vits_bedrock"] = (f"{(vb_br or 0) - vs_br:+.2f}"
                                        if (vb_br is not None and vs_br is not None) else "N/A")
        rows.append(row)
...
def main():
    parser = argparse.ArgumentParser(description="DINOv3 ViT-B/16 few-shot terrain probe")
    ...
    cache_test_feats  = os.path.join(CACHE_DIR, "dinov3_vitb_test_287_feats.npy")
    cache_test_labels = os.path.join(CACHE_DIR, "dinov3_vitb_test_287_labels.npy")
    cache_train_feats = os.path.join(CACHE_DIR, "dinov3_vitb_train_1000_feats.npy")
    cache_train_labels= os.path.join(CACHE_DIR, "dinov3_vitb_train_1000_labels.npy")
    ...
    if use_cache:
        ...
        print(f"  Features cached → {CACHE_DIR}/dinov3_vitb_*")
    ...
    print("\n=== FINAL RESULT — D2 DINOv3 ViT-B/16 ===")
    if 1000 in results:
        vb_ov = results[1000]["overall"]
        vs_ov = DINOV3_VITS_BASELINES[1000]["overall"]
        vl_ov = DINOV3_VITL_BASELINES[1000]["overall"]
        ...
        status = "✅ IMPROVED vs ViT-S" if vb_ov > vs_ov else "❌ NO IMPROVEMENT vs ViT-S"
        print(f"  1000-shot Overall: ViT-S {vs_ov:.2f}% → ViT-B {vb_ov:.2f}%  "
              f"(Δ {vb_ov - vs_ov:+.2f}%)  |  ViT-L {vl_ov:.2f}%  → {status}")
        print(f"  1000-shot Bedrock: ViT-S {vs_br:.2f}% → ViT-B {vb_br:.2f}%  "
              f"(Δ {vb_br - vs_br:+.2f}%)")
        print(f"  Gap vs supervised (96.67%): {vb_ov - SUPERVISED_BASELINE:+.2f}%")
        print(f"  Scaling position: ViT-S {vs_ov:.1f}% < ViT-B {vb_ov:.1f}% {'<' if vb_ov < vl_ov else '>'} ViT-L {vl_ov:.1f}%")
