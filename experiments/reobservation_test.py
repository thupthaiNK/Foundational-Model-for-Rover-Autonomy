#!/usr/bin/env python3
"""
Purpose: Live recorder for "A2 re-observation mode" (scoped via grill-thesis
         2026-07-19, item 7 of the L1-L6 further-work plan): the costmap
         keeps a per-cell latest-write confidence store, and on frontier
         exhaustion the planner autonomously revisits the known non-hazard
         cell with the LOWEST recorded confidence, one cell at a time.
         Success criteria fixed BEFORE this official run, per this thesis's
         standing practice, calibrated from a real smoke test (2026-07-19,
         reobs_smoke1.log): exploration exhausted the 961-cell box in
         1085.7 s (42 selections, 100% coverage), the transition fired
         autonomously, and 20 re-observation selections followed at
         ~31.7 s/revisit with the selection invariant holding 20/20 -- so
         the cap and count below have real margin, not guesses.
         PRIMARY (pass/fail, all three required):
           P1. The exploration -> re-observation transition fires
               autonomously (parsed from the planner's own "entering
               re-observation mode" /rosout line) within MAX_DURATION_S.
           P2. Zero invariant violations: every "Reobservation N selected"
               line must report selected_conf == pool_min. The planner
               recomputes pool_min independently of the selector, so any
               violation is a genuine bug, not a filtering artefact --
               a logical guarantee of the lowest-first design (the value,
               not the cell identity: ties are legal, broken by distance).
           P3. >= 10 re-observation selections within the cap, so P2 is
               exercised enough to be meaningful evidence rather than a
               vacuous pass (smoke pace projects ~20 at this cap).
         SECONDARY (descriptive, NOT pass/fail -- latest-write does NOT
         guarantee a revisit raises confidence, and pre-registering a
         direction would gamble on physics, the frontier-v1 lesson):
         per-revisited-cell confidence delta (value at selection vs final
         grid value at cap), counts raised/lowered/unchanged, frontier
         selections, coverage, path length, replans, and how often
         grid_with_start_freed's escape fired (its first live activations
         ever were observed in this arena's smoke test).
Inputs:  /exomy/odom (nav_msgs/Odometry) -- ground truth
         ...
         /l5_lite_frontier_goal, /l5_lite_reobserve_goal (PointStamped) --
             ...
             /rosout log which is authoritative (H17 found the topic
             channel can undercount under load, §4.8.29)
         /l5_lite_plan (nav_msgs/Path) -- replan counting
         /rosout (rcl_interfaces/Log) -- P1/P2/P3 evidence
Outputs: experiments/results/reobservation_test.csv
         experiments/results/reobservation_selections.csv
         experiments/results/figures/reobservation_test.png
How to run:
    # Terminal 1:
    source /opt/ros/humble/setup.bash && source ros2_ws/install/setup.bash
    ros2 launch simulation/launch/reobservation_test.launch.py

    # Terminal 2 (after l5_lite_planner_node has started):
    source /opt/ros/humble/setup.bash && source ros2_ws/install/setup.bash
    python3 experiments/reobservation_test.py
...
class ReobservationRecorder(Node):

    def __init__(self):
        super().__init__("reobservation_test")
        self._t0 = time.time()
        self._box = BOX_CELLS
...
def main():
    ...
    # -- Save raw records ----------------------------------------------------
    csv_path = os.path.join(RESULTS_DIR, "reobservation_test.csv")
    ...
    sel_path = os.path.join(RESULTS_DIR, "reobservation_selections.csv")
    ...
    fig_path = os.path.join(FIGURES_DIR, "reobservation_test.png")
    fig.tight_layout()
    ...
    print("\n" + "=" * 70)
    print("A2 RE-OBSERVATION TEST SUMMARY")
