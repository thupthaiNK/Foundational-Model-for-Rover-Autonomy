"""
Purpose: Compute the confusion matrix and t-SNE 2D embedding for the
         deployed comparison model (DINOv2 ViT-L/14, 1000-shot), from the
         already-cached frozen features, and export as plain CSVs. This is
         a data-preparation step only, no plotting -- the actual figures are
         drawn in MATLAB (experiments/make_thesis_figures_2.m) per this
         thesis's tooling convention (data charts -> MATLAB, feature
         extraction/ML -> Python).
Inputs:  experiments/results/feature_cache/dinov2_vitl_{train_1000,test_287}_{feats,labels}.npy
Outputs: experiments/results/confusion_matrix_dinov2_vitl.csv
         experiments/results/tsne_dinov2_vitl.csv
How to run:
    python3 experiments/prep_confusion_tsne_data.py
...
train_X = normalize(load("dinov2_vitl_train_1000_feats.npy"))
train_y = load("dinov2_vitl_train_1000_labels.npy")
test_X = normalize(load("dinov2_vitl_test_287_feats.npy"))
test_y = load("dinov2_vitl_test_287_labels.npy")
...
cm_path = os.path.join(OUT_DIR, "confusion_matrix_dinov2_vitl.csv")
...
tsne_path = os.path.join(OUT_DIR, "tsne_dinov2_vitl.csv")
