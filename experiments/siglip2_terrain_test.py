"""
Purpose: Few-shot linear probe on frozen SigLIP2-B/16 features using AI4Mars dataset.
         SigLIP2 (Tschannen et al., 2025) is a CLIP-family model trained with sigmoid
         pairwise loss instead of softmax contrastive loss, using improved data and
         training recipes over the original SigLIP. Tests whether a stronger CLIP-family
         model narrows the gap to DINO self-distillation on Mars terrain classification.
...
Outputs: Per-class accuracy by shot count, CSV saved to results/
         Feature cache saved to results/feature_cache/siglip2_*
How to run:
    python3 -u experiments/siglip2_terrain_test.py | tee /tmp/siglip2_log.txt
    python3 -u experiments/siglip2_terrain_test.py --shots 1000  # 1000-shot only
    python3 -u experiments/siglip2_terrain_test.py --shots 10    # smoke test
Note: Uses HuggingFace transformers (SiglipModel). Model ~344MB download on first run.
Reference: Tschannen et al. (2025) SigLIP 2: Multilingual Vision-Language Encoders with
           Improved Semantic Understanding. arXiv:2502.14786.
...
def load_model(device: str):
    print(f"Loading SigLIP2-B/16 vision encoder via transformers  [{MODEL_ID}]")
    print(f"  Pretraining: CLIP-family sigmoid pairwise loss (SigLIP2)")
    print(f"  Expected: ~93M params, 768-d pooler_output, 224×224 input")
...
def extract_features(model, processor, image_paths, labels, device: str, desc: str = ""):
    """Extract L2-normalised SigLIP2 pooler_output features (768-d)."""
    features = []
    n  = len(image_paths)
    t0 = time.perf_counter()
    ...
    for i, path in enumerate(image_paths):
        ...
        with torch.no_grad():
            ...
            feat        = out.pooler_output.cpu().numpy().squeeze()  # [768]
...
def print_results(results: dict):
    ...
    print("\n" + "=" * 110)
    print("SigLIP2-B/16 Few-Shot Linear Probe — AI4Mars (287 test images)")
    print("CLIP-family (sigmoid loss) vs DINO self-distillation: DINOv2-B (91.3%), DINOv3-L (92.3%)")
    print("=" * 110)

    for metric in ["overall"] + CLASS_NAMES:
        row = f"{metric:<10}"
        for s in shots:
            s_val  = results[s]["overall"] if metric == "overall" else results[s]["per_class"].get(metric)
            ...
            s_str  = f"{s_val:>6.1f}%"  if s_val is not None else "   N/A"
            v2b_str= f"{v2b:>6.1f}%"   if v2b   is not None else "   N/A"
            v3l_str= f"{v3l:>6.1f}%"   if v3l   is not None else "   N/A"
            d_v2b  = f"{(s_val-v2b):>+6.1f}%" if (s_val is not None and v2b) else "    N/A"
            row   += f"  [{s:>4}shot] SLP2:{s_str} v2B:{v2b_str} Δ:{d_v2b} v3L:{v3l_str}"
        print(row)

    print("=" * 110)
    print("\nBedrock — SigLIP2 vs DINO self-distillation:")
    for s in shots:
        s_b  = results[s]["per_class"].get("bedrock")
        v2b_b= DINOV2_VITB_BASELINES.get(s, {}).get("bedrock")
        if s_b is not None and v2b_b is not None:
            print(f"  {s:>5}-shot:  v2B {v2b_b:.1f}%  →  SigLIP2 {s_b:.1f}%  "
                  f"(Δ {s_b-v2b_b:+.1f}%)")

    if 1000 in results:
        s_ov  = results[1000]["overall"]
        v2b_ov= DINOV2_VITB_BASELINES[1000]["overall"]
        v3l_ov= DINOV3_VITL_BASELINES[1000]["overall"]
        print(f"\n>>> 1000-shot: v2B {v2b_ov:.2f}%  SigLIP2 {s_ov:.2f}%  v3L {v3l_ov:.2f}%  "
              f"|  Gap vs supervised: {s_ov - SUPERVISED_BASELINE:+.2f}%")


def save_csv(results: dict, avg_ms_extract: float):
    os.makedirs(RESULTS_DIR, exist_ok=True)
    path  = os.path.join(RESULTS_DIR, "siglip2_terrain_few_shot.csv")
    shots = sorted(results.keys())
...
def main():
    parser = argparse.ArgumentParser(description="SigLIP2-B/16 few-shot terrain probe")
    ...
    cache_test_feats   = os.path.join(CACHE_DIR, "siglip2_test_287_feats.npy")
    cache_test_labels  = os.path.join(CACHE_DIR, "siglip2_test_287_labels.npy")
    cache_train_feats  = os.path.join(CACHE_DIR, "siglip2_train_1000_feats.npy")
    cache_train_labels = os.path.join(CACHE_DIR, "siglip2_train_1000_labels.npy")
    ...
    if use_cache:
        ...
        est_min = len(train_sampled) * ms_per_img / 1000 / 60
        print(f"  Estimated time: {est_min:.0f}min at {ms_per_img:.0f}ms/img")
        ...
        print(f"  Features cached → {CACHE_DIR}/siglip2_*")
    ...
    print("\n=== FINAL RESULT — SigLIP2-B/16 ===")
    if 1000 in results:
        s_ov  = results[1000]["overall"]
        v2b_ov= DINOV2_VITB_BASELINES[1000]["overall"]
        v3l_ov= DINOV3_VITL_BASELINES[1000]["overall"]
        s_br  = results[1000]["per_class"].get("bedrock", 0)
        v2b_br= DINOV2_VITB_BASELINES[1000].get("bedrock", 0)
        clip_baseline = 87.8
        status = "✅ BEATS DINOv2-B" if s_ov > v2b_ov else "❌ BELOW DINOv2-B"
        print(f"  1000-shot Overall: DINOv2-B {v2b_ov:.2f}% vs SigLIP2 {s_ov:.2f}%  "
              f"(Δ {s_ov-v2b_ov:+.2f}%)  → {status}")
        print(f"  1000-shot Bedrock: DINOv2-B {v2b_br:.2f}% vs SigLIP2 {s_br:.2f}%  "
              f"(Δ {s_br-v2b_br:+.2f}%)")
        print(f"  vs CLIP ViT-B/32 baseline: {s_ov:.2f}% vs {clip_baseline:.2f}%  "
              f"(Δ {s_ov-clip_baseline:+.2f}%)")
        print(f"  vs DINOv3 ViT-L (best): {s_ov:.2f}% vs {v3l_ov:.2f}%  (Δ {s_ov-v3l_ov:+.2f}%)")
        print(f"  Gap vs supervised (96.67%): {s_ov - SUPERVISED_BASELINE:+.2f}%")
        paradigm = "DINO self-distillation (DINOv2-B)" if s_ov < v2b_ov else "SigLIP2-B/16"
        print(f"  CLIP-vs-DINO verdict: {paradigm} wins for Mars terrain")
